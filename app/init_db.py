...
if __name__ == '__main__':
    try:
        ...
        sql = (
            "CREATE INDEX IF NOT EXISTS index_pair_id ON check_result (pair_id);")
        cursor.execute(sql)
        sqlite_connection.commit()

        sql = '''create table IF NOT EXISTS user_is_blocked(
                id INTEGER PRIMARY KEY,
                user TEXT,
                date TEXT
                )
        ...
        # with open("files/34.ba","rt") as bf:
        #     with open("files/34.ru", "rt") as rf:
